...
test("패널 최초 로드 안내 배너는 가드레일 보조 도구 문구를 포함한다", () => {
  const { context, createdElements } = createContentHarness();

  vm.runInContext(
    `createPanel({ user: { email: "guardrail@test.test" } })`,
    context,
  );

  const panelMarkup = createdElements[0].innerHTML;
  assert.match(panelMarkup, /data-panel-intro-notice/);
  assert.match(panelMarkup, /가드레일이 활성화되어 있어요/);
  assert.match(panelMarkup, /수익률을 보장하는 서비스가 아니라/);
  assert.match(panelMarkup, /내가 정한 원칙을 주문 순간에 확인/);
});
...
test("upbit-element fixture의 안정 data-testid를 주문 parser가 우선 사용한다", () => {
  const fixture = fs.readFileSync(
    path.join(__dirname, "../docs/upbit-element.md"),
    "utf8",
  );
  const requiredTestIds = [
    ...
    "volume-input",
    "order-price-input",
    "percentage-button-10",
    "percentage-button-25",
    "percentage-button-50",
    "percentage-button-100",
    "popupWrapper",
  ];

  for (const testId of requiredTestIds) {
    assert.match(fixture, new RegExp(`data-testid="${testId}"`));
  }
  ...
  const result = vm.runInContext(
    `(() => {
      const panel = new Element();
      panel.tagName = "DIV";
      panel.textContent = "시장가 주문가능 주문총액";
      panel.dataset = {};
      panel.className = "";
      panel.parentElement = document.body;
      panel.getAttribute = () => null;
      ...
      amountInput.parentElement = panel;
      amountInput.getAttribute = (name) =>
        name === "data-testid" ? "total-input" : null;

      const quantityInput = new HTMLInputElement();
      quantityInput.value = "0.00005715";
      quantityInput.labels = [];
      quantityInput.parentElement = panel;
      quantityInput.getAttribute = (name) =>
        name === "data-testid" ? "volume-input" : null;

      const priceInput = new HTMLInputElement();
      priceInput.value = "95,219,000";
      priceInput.labels = [];
      priceInput.parentElement = panel;
      priceInput.getAttribute = (name) =>
        name === "data-testid" ? "order-price-input" : null;

      const inputs = [amountInput, quantityInput, priceInput];
      panel.querySelector = (selector) => {
        const match = selector.match(/data-testid="([^"]+)"/);
        if (!match) return null;
        return inputs.find((input) => input.getAttribute("data-testid") === match[1]) || null;
      };
      panel.querySelectorAll = (selector) => selector === "input" ? inputs : [];

      const modal = new Element();
      modal.tagName = "DIV";
      modal.textContent = "매수주문 확인 시장가 매수 DOGE/KRW 총액 5,000 KRW 취소 매수 확인";
      modal.dataset = {};
      modal.className = "";
      ...
      button.closest = (selector) => {
        ...
        if (selector.includes("#modal") || selector.includes("[data-testid='popupWrapper']")) {
          return modal;
        }
        return null;
      };

      behaviorState = { market: "KRW-DOGE" };
      const extraction = buildUpbitOrderExtraction({
        button,
        formRoot: panel,
        modalRoot: modal
      });
  ...
  assert.equal(result.market, "KRW-DOGE");
  assert.equal(result.side, "BUY");
  assert.equal(result.orderMode, "MARKET");
  assert.equal(result.intentAmount, 5000);
  assert.equal(result.intentPrice, null);
});

test("실제 Upbit fixture의 시장가 매도 확인 모달은 수량을 추출한다", () => {
  const fixture = fs.readFileSync(
    path.join(__dirname, "../docs/upbit-element.md"),
    "utf8",
  );
  assert.match(fixture, /매도주문 확인/);
  assert.match(fixture, /0\.00005715 BTC/);

  const { context } = createContentHarness();
  const snapshot = runUpbitConfirmScenario(context, {
    buttonText: "매도 확인",
    modalText: "매도주문 확인 시장가 매도 BTC/KRW 주문수량 0.00005715 BTC 취소 매도 확인",
  });

  assert.equal(snapshot.side, "SELL");
  assert.equal(snapshot.orderMode, "MARKET");
  assert.equal(snapshot.intentQuantity, "0.00005715");
