...
test("최근 AI 인사이트 카드는 내부 상태와 legacy 문구를 표시용 ViewModel에서 정리한다", () => {
  ...
  assert.equal(overview.includes("이번 주 기록에서 발견한 패턴"), true);
  assert.equal(overview.includes("다음 주에 확인할 원칙"), true);
  assert.equal((overview.match(/primaryCard \?/g) || []).length, 1);
...
test("최근 AI 인사이트 정량 지표는 실제 값 우선이고 내부 상태 코드를 렌더링하지 않는다", () => {
  ...
  assert.equal(overview.includes("24시간 가상 주문 결과"), true);
  assert.equal(overview.includes("비교 가능한 24시간 가격 효과가 아직 없어요."), true);
  assert.equal(overview.includes("formatKrw(virtualMetric.netValue)"), true);
...
test("Extension은 주간 status만 조회하고 생성 API 없이 대시보드 query로 안내한다", () => {
  ...
  assert.equal(combined.includes("/api/insights/weekly/generate"), false);
  assert.equal(combined.includes("GENERATE_DAILY_INSIGHT"), false);
  assert.equal(combined.includes("GENERATE_WEEKLY_INSIGHT"), false);
  assert.match(content, /GET_WEEKLY_INSIGHT_STATUS/);
  assert.match(content, /\/dashboard\/ai-insights\?week=/);
  assert.match(content, /focus=generate/);
  assert.equal(content.includes("GET_DAILY_INSIGHT_STATUS"), false);
});

test("AI 인사이트 페이지는 저장된 주간 리포트 목록과 데스크톱 모달을 사용한다", () => {
  ...
  assert.match(page, /\/api\/insights\/weekly\/\$\{encodeURIComponent\(report\.weekKey\)\}\/suggestions/);
