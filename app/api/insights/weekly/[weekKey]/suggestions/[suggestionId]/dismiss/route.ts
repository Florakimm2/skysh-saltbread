...
import { dismissWeeklyInsightSuggestion } from "@/backend/modules/insight/weekly-service";
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await dismissWeeklyInsightSuggestion({ userId, weekKey, suggestionId });
    return ok(result);
  } catch (error) {
